...
# 超參數設置
learning_rate = 0.00001  # 調整學習率以穩定訓練過程
epochs = 200           # 增加訓練迭代次數
batch_size = 32
...
with strategy.scope():
    model = tf.keras.Sequential([
        tf.keras.layers.Dense(256, activation='relu'),   # 增加層數和神經元數量
        tf.keras.layers.Dropout(0.3),                    # 增加 Dropout 層來防止過擬合
    ...
    def train_model():
        ...
        train_dataset = tf.data.Dataset.from_tensor_slices((train_data, train_label)).shuffle(10000).batch(batch_size)  # 增加 shuffle
